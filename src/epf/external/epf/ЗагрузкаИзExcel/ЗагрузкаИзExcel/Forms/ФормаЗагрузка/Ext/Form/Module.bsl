...
Процедура ПриОткрытии(Отказ)
	...
	ВыбратьКлючНастроек(Ложь, Ложь, КлючНастроекОсновной);
...
Функция ТаблицаДанныхExcelОтладкаТаблицаНаСервере(ТолькоПомеченные = Истина)
	...
	Если ТолькоПомеченные Тогда
		ОбъектДанных = Объект.ТаблицаДанныхExcelОбъект.Выгрузить(Новый Структура("Пометка", Истина))
	Иначе
		ОбъектДанных = Объект.ТаблицаДанныхExcelОбъект.Выгрузить()
	КонецЕсли;
...
Процедура ТаблицаДанныхExcelЗагрузитьИзТекстаНаСервере(ДанныеСтр)
	...
	ОбъектОбработка.ДесериализоватьОбъектТаблицыВЯчейках(ОбъектДанных, ТаблицаНомеровКолонок);
...
Процедура ХранилищеОН_СохранитьВсеНаСервере(ЭтоПолучениеДанных, КлючНастроекПараметр, ОписаниеНастройки)
	...
	Если НЕ ЭтоПолучениеДанных Тогда
		...
		Если Настройки_СохранятьТаблицаДанныхExcel Тогда
			...
			// Преобразуем "таблицы в ячейках" из строки в таблицу.
			ОбъектОбработки().СериализоватьОбъектТаблицыВЯчейках(ОбъектДанных, ТаблицаНомеровКолонок);
			ХранилищеОбщихНастроек.Сохранить(ИмяОбработки, "ТаблицаДанныхExcel_" + КлючНастроекПараметр, ОбъектДанных);
		Иначе
...
Функция ХранилищеОН_ВосстановитьВсеНаСервере(ЭтоПолучениеДанных, КлючНастроекПараметр, НеВосстанавливатьОбщие = Ложь)
	...
	Если НЕ ЭтоПолучениеДанных Тогда
		...
		Если Настройки_СохранятьТаблицаДанныхExcel Тогда
			...
			ОбъектДанных = ХранилищеОбщихНастроек.Загрузить(ИмяОбработки, "ТаблицаДанныхExcel_" + КлючНастроекПараметр);
			Если ТипЗнч(ОбъектДанных) = Тип("ТаблицаЗначений") Тогда
				// Преобразуем "таблицы в ячейках" из таблиц в строки.
				ОбъектОбработка.ДесериализоватьОбъектТаблицыВЯчейках(ОбъектДанных, ТаблицаНомеровКолонок);
				
				Объект.ТаблицаДанныхExcelОбъект.Очистить();
				Для Каждого ДанныеСтроки Из ОбъектДанных Цикл
...
&НаКлиенте
Функция ИмяПоПредставлениюНаКлиенте(Представление) Экспорт
	...
	Если ДополнительныеПараметры.Действие = "КлючНастроекОсновнойНачалоВыбора" Тогда
		...
		Иначе
			...
			ИначеЕсли ДополнительныеПараметры.Событие = "ИмяФайлаЗагрузкиМакетПриИзменении" Тогда
				...
				ВыбратьКлючНастроек(Истина, Истина);
			...
			ИначеЕсли ДополнительныеПараметры.Событие = "ИмяФайлаЗагрузкиНачалоВыбора" Тогда
				...
				ВыбратьКлючНастроек(Истина, Истина,,,Истина);
			...
			ИначеЕсли ДополнительныеПараметры.Событие = "РежимЧтенияДанныхПриИзменении" Тогда
				НоваяНастройкаОбработка(КодВозвратаДиалога.Да, Неопределено);
				ВыбратьКлючНастроек(Истина, Ложь);
			...
			ИначеЕсли ДополнительныеПараметры.Событие = "ТаблицаИсторияВыбрать" Тогда
				Если НЕ Элементы.ТаблицаИстория.ТекущиеДанные = Неопределено Тогда
					ВыбратьКлючНастроек(Ложь, Ложь, Элементы.ТаблицаИстория.ТекущиеДанные.КлючНастроек);
				КонецЕсли;
			...
			ИначеЕсли ДополнительныеПараметры.Событие = "ТаблицаИсторияСохранитьВсеНастройки" Тогда
				...
				Если ДиалогОткрытияФайла.Выбрать() Тогда
					...
					Для Каждого СтрокаИстория Из ТаблицаИстория Цикл
						ХранилищеОН_ВосстановитьВсеНаСервере(Ложь, СтрокаИстория.КлючНастроек);
			...
			ИначеЕсли ДополнительныеПараметры.Событие = "ТаблицаИсторияВосстановитьВсеНастройки" Тогда
				...
				ДиалогОткрытияФайла.Заголовок          = "Выберите каталог";
				Если ДиалогОткрытияФайла.Выбрать() Тогда
					Для Каждого ТекФайл Из НайтиФайлы(ДиалогОткрытияФайла.Каталог, "НастройкиФормыОбщие_*.xml") Цикл
						ТекКлючНастроек = СтрЗаменить(ТекФайл.ИмяБезРасширения, "НастройкиФормыОбщие_", "");
						ЭтаФорма.КлючНастроекОсновной = ТекКлючНастроек;
						ВыбратьКлючНастроек(Ложь,Ложь,,Истина);
						ВосстановитьВсеНастройкиОбработка(ТекКлючНастроек, ДиалогОткрытияФайла.Каталог);
						ХранилищеОН_СохранитьВсеНаСервере(Ложь,ТекКлючНастроек,ОписаниеНастройки());
					КонецЦикла;
					ТаблицаИсторияОбновитьНаСервере();
					ВыбратьКлючНастроек(Ложь, Ложь);
					ТаблицаИсторияУстановитьТекущую(ТаблицаИстория);
			...
			ИначеЕсли ДополнительныеПараметры.Событие = "ТаблицаИсторияСкопировать" Тогда
				...
				ТаблицаИсторияОбновитьНаСервере();
				ВыбратьКлючНастроек(Ложь, Ложь, КлючНастроекПриемник);
...
&НаКлиенте
Процедура ВыбратьКлючНастроек(РежимПоискаИстории, ЭтоПолучениеДанных, КлючНастроекОсновнойВыбрано = Неопределено, НеОбновлятьИзХранилища = Ложь, НеВосстанавливатьОбщие = Ложь) Экспорт
...
Функция ОписаниеНастройки() Экспорт
	...
	Если ЭтаФорма.РежимЧтенияДанных = "По макету" Тогда
		ТекОписаниеТекст = ПодставитьПараметрыВСтроку("Макет %1 строк",ЭтаФорма.ТабДокОбъект.ВысотаТаблицы);
	Иначе
		ТекОписаниеТекст = ПодставитьПараметрыВСтроку("Файл,лист %1", ЭтаФорма.КлючНастроекОсновной);
	КонецЕсли;
	ТекОписание = ПодставитьПараметрыВСтроку(ТекОписание,
...
Процедура ОбработатьВыборФайла_МХЛ(Результат, Адрес, ВыбранноеИмяФайла, ДополнительныеПараметры) Экспорт
	...
	Если Результат Тогда
		...
		ВыбратьКлючНастроек(Истина, Ложь,,,Истина);
...
Процедура ТаблицаДанныхExcelОткрытие(Элемент, СтандартнаяОбработка)
	...
	ИмяПоля = Сред(Элемент.Имя,СтрДлина("ТаблицаДанныхExcelОбъект")+1);
