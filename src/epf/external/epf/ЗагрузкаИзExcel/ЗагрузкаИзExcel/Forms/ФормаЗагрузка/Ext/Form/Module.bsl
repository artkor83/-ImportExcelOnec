...
Процедура ИмяФайлаЗагрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	//ВопросЗавершение(КодВозвратаДиалога.Нет, Новый Структура("Действие,Событие", "КлючНастроекОсновнойНачалоВыбора", "ИмяФайлаЗагрузкиНачалоВыбора))
...
&НаСервере
Процедура ХранилищеОН_ВосстановитьВсеНаСервере(ЭтоПолучениеДанных, КлючНастроекПараметр)
...
Процедура ВосстановитьВсеНастройки(Команда)
	...
	ДиалогОткрытияФайла.Заголовок          = "Выберите каталог";
	Если ДиалогОткрытияФайла.Выбрать() Тогда
		ВосстановитьВсеНастройкиОбработка(ДиалогОткрытияФайла.Каталог);
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
Процедура ВосстановитьВсеНастройкиОбработка(Каталог)

	ФайлыНайденыВКаталоге = Ложь;
	
	КлючНастроек = "НастройкиФормыОбщие_" + ЭтаФорма.КлючНастроекОсновной + ".xml";
	ТекстФ = Новый ТекстовыйДокумент;
	...
	КлючНастроек = "ТаблицаПараметры_" + ЭтаФорма.КлючНастроекОсновной + ".xml";
	ТекстФ = Новый ТекстовыйДокумент;
	Файл = Новый файл(Каталог +"\"+ КлючНастроек);
	Если Файл.Существует() Тогда
		ТекстФ.Прочитать(Каталог +"\"+ КлючНастроек);
	...
	КлючНастроек = "ТаблицаНомеровКолонок_" + ЭтаФорма.КлючНастроекОсновной + ".xml";
	Файл = Новый файл(Каталог +"\"+ КлючНастроек);
	Если Файл.Существует() Тогда
		ТекстФ = Новый ТекстовыйДокумент;
		ТекстФ.Прочитать(Каталог +"\"+ КлючНастроек);
	...
	Если Настройки_СохранятьТаблицаДанныхExcel Тогда
		...
		КлючНастроек = "ТаблицаДанныхExcel_" + ЭтаФорма.КлючНастроекОсновной + ".txt";
		ТекстДок = Новый ТекстовыйДокумент;
		Файл = Новый файл(Каталог +"\"+ КлючНастроек);
		Если Файл.Существует() Тогда
...
Процедура СохранитьВсеНастройки(Команда)
	...
	Если ДиалогОткрытияФайла.Выбрать() Тогда
		...
		Если РежимЧтенияДанных = "По макету" И ТабДокОбъект.ВысотаТаблицы > 0 Тогда
			КлючНастроек = ЭтаФорма.КлючНастроекОсновной + ".mxl";
		...
		Если Настройки_СохранятьТаблицаДанныхExcel Тогда
			...
			ТекстДок = Новый ТекстовыйДокумент;
			ТекстДок.УстановитьТекст(Текст);
			ТекстДок.Записать(ДиалогОткрытияФайла.Каталог +"\"+ КлючНастроек);
...
&НаСервере
Процедура ХранилищеОН_СохранитьВсеНаСервере(ЭтоПолучениеДанных, ОписаниеНастройки, КлючНастроекПараметр)
...
Процедура СохранениеНастройкиДиалог(Отказ, Событие)
	...
	//	  ИЛИ (Событие = "ПередЗакрытием" И НЕ ПустаяСтрока(КлючНастроекОсновной))
	//	 ) Тогда
	Если Модифицированность И (
		  Не ПустаяСтрока(Параметры.КлючНастроекОсновной)
	...
	Если ДополнительныеПараметры.Действие = "КлючНастроекОсновнойНачалоВыбора" Тогда
		...
		Иначе
			...
			ИначеЕсли ДополнительныеПараметры.Событие = "ИмяФайлаЗагрузкиНачалоВыбора" Тогда
				...
				ВосстановитьВсеНастройкиОбработка(Файл.Путь);
...
&НаКлиенте
Процедура ОбновитьКлючНастроекОсновной(РежимПоискаИстории, ЭтоПолучениеДанных, КлючНастроекОсновнойВыбрано = Неопределено, НеОбновлятьИзХранилища = Ложь) Экспорт
	...
	Если НЕ НеОбновлятьИзХранилища Тогда
		ХранилищеОН_ВосстановитьВсе(ЭтоПолучениеДанных);
	КонецЕсли;
...
Процедура ХранилищеОН_СохранитьВсе(ЭтоПолучениеДанных = Ложь) Экспорт
	...
	ХранилищеОН_СохранитьВсеНаСервере(ЭтоПолучениеДанных, ОписаниеНастройки(), ЭтаФорма.КлючНастроекОсновной);
...
&НаКлиенте
Процедура ХранилищеОН_ВосстановитьВсе(ЭтоПолучениеДанных = Ложь) Экспорт
	
	Если ПустаяСтрока(ЭтаФорма.КлючНастроекОсновной) Тогда
		Возврат
	КонецЕсли;
	
	ХранилищеОН_ВосстановитьВсеНаСервере(ЭтоПолучениеДанных, ЭтаФорма.КлючНастроекОсновной)
