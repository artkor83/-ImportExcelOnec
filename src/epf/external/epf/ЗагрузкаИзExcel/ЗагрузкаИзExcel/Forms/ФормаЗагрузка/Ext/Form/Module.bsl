...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Параметры.АлгоритмыСтрокой = "Алгоритм_ПоискДанныхСтроки,Алгоритм_ЗагрузкаДанныхСтроки,Алгоритм_ПередЗагрузкойДанных";
...
&НаКлиенте
Процедура ТаблицаНомеровКолонокФормаПоискТипов(Команда)
...
Процедура ТаблицаНомеровКолонокТипЗначенияПредставлениеПриИзменении(Элемент)
	...
	ТипЗначенияПредставление = ""; ТипЗначения = ТекущиеДанные.ТипЗначения;
	ТаблицаНомеровКолонокНаСервере(ТекущиеДанные.ТипЗначенияПредставление, ТипЗначенияПредставление, ТипЗначения);
	
	ТекущиеДанные.ТипЗначения = ТипЗначения;
	ТекущиеДанные.ТипЗначенияПредставление = ТипЗначенияПредставление;
	
КонецПроцедуры

&НаСервере
Процедура ТаблицаНомеровКолонокНаСервере(ПредставлениеТипов, ТипЗначенияПредставление, ТипЗначения)
...
Функция ТаблицаДанныхExcelОтладкаТаблицаНаСервере()
	Возврат ОбъектОбработки().СериализоватьОбъект(Объект.ТаблицаДанныхExcelОбъект.Выгрузить())
КонецФункции
...
Процедура ПолучитьДанные_Обработка_МХЛ()
	...
	Пока Истина Цикл
		...
		Иначе
			ЗначениеИндикатора = (100 * НомПП/(НомерКонец - НомерНачало));
		КонецЕсли;
...
Процедура ПолучитьДанныеExcelНаСервере(ПорцияДанных)
	...
	ПараметрыЗагрузкиДанных = Новый Структура("Алгоритм_ПоискДанныхСтроки", Алгоритм_ПоискДанныхСтроки);
	...
	ПорцияДанныхОбработано = ОбъектОбработки().ПолучитьДанныеExcel(
		ПорцияДанных, ТаблицаНомеровКолонок.Выгрузить(), ПараметрыЗагрузкиДанных);
...
Процедура ЗагрузитьДанныеExcel_Основной(ПараметрыЗагрузкиДанных)
	...
	КоличествоВсе = Объект.ТаблицаДанныхExcelОбъект.Количество();
...
Процедура ЗагрузитьДанныеExcel_ОсновнойНаСервере(ПараметрыЗагрузкиДанных)
	...
	ПараметрыЗагрузкиДанных.ТаблицаДанныхExcel_Адрес = ПоместитьВоВременноеХранилище(Объект.ТаблицаДанныхExcelОбъект.Выгрузить(),
...
Процедура ЗагрузитьВсеНастройкиХранилищеНаСервере(ЭтоПолучениеДанных)
	...
	ЗарплатаКадры.ЗаполнитьМассивИменРеквизитовФормы(ЭтаФорма, СуществующиеРеквизиты, "Объект.ТаблицаДанныхExcelОбъект");
	...
	ЗарплатаКадры.ИзменитьРеквизитыФормы(ЭтаФорма, МассивРеквизитов, Новый Массив, СуществующиеРеквизиты);
	
	ОбъектОбработка.СоздатьЭлементыФормыПоТаблицеНомеровКолонок(ЭтаФорма, "ТаблицаДанныхExcelОбъект", ТаблицаНомеровКолонок);
	//ЗначениеВРеквизитФормы(ТаблицаДанныхExcelОбъект,    "ТаблицаДанныхExcel");
...
Процедура УстановитьВсе(Команда)
	...
	//Для Каждого ТекПодч ИЗ ТаблицаДанныхExcel.ПолучитьЭлементы() Цикл
...
Функция ПараметрыЗагрузкиДанных()
	...
	ПараметрыЗагрузкиДанных = Новый Структура(
		...
		Алгоритм_ПоискДанныхСтроки, Алгоритм_ЗагрузкаДанныхСтроки, Алгоритм_ПередЗагрузкойДанных);
