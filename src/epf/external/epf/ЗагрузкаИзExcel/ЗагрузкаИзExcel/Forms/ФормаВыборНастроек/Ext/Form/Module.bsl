...
&НаКлиенте
Функция КлючЭталон() Экспорт
	
	КлючЭталон = ВладелецФормы.КлючНастроекОсновнойПоУмолчанию();
...
&НаКлиенте
Процедура ОбновитьИмяОбработка() Экспорт
	
	КлючЭталон = КлючЭталон();
	Если ПустаяСтрока(КлючЭталон) Тогда
		КлючНастроекНовый = "";
		Возврат
	КонецЕсли;
